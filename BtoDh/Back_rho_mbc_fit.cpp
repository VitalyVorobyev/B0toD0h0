...
//  gROOT->ProcessLine(".L pdfs/RooStudentsGauss1D.cxx+");
  ...
  RooRealVar mbc0("mbc0","mbc0",5.286,5.26,5.30);
...
//   RooRealVar sg("sg","sg",1.,0.9,1.1); sg.setConstant(kTRUE);
  RooRealVar sl("sl","sl",0.0135,0.,0.5);
  RooRealVar sr("sr","sr",0.001,0.,0.5);
...
//  RooStudentsGauss1D pdf("pdf","pdf",mbc,mbc0,w,dw,nh,nl,f,dmbc0,sg);
  ...
  RooRealVar sll("sll","sll",0.0087,0.,0.5);
  RooRealVar srr("srr","srr",0.0028,0.,0.5);
...
//  pdf.fitTo(ds,Range("fit"),Verbose(),Timer(true));
  ...
  TCanvas* cm = new TCanvas("M_{bc}, peaking background","M_{bc}, peaking background",600,700);
  cm->cd();
