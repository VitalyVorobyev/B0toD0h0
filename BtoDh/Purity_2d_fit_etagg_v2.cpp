...
//  else RooProdPdf pdf_sig("pdf_sig","pdf_sig",pdf_de_sig,Conditional(pdf_mbc,mbc));
  ...
  RooChebychev pdf_de_bb_comb("pdf_de_bb_comb","pdf_de_bb_comb",de,RooArgSet(d1,d2));
  ...
  RooFormulaVar NCmb("NCmb","@0+@1",RooArgSet(Ncmb,NBBcmb));

  RooAddPdf pdf_de("pdf_de","pdf_de",RooArgList(pdf_de_sig,pdf_de_comb),RooArgList(Nsig,NCmb));
  RooAddPdf pdf_mbc("pdf_mbc","pdf_mbc",RooArgList(pdf_mbc_sig,pdf_mbc_comb,pdf_mbc_bb_comb),RooArgList(Nsig,Ncmb,NBBcmb));
  RooProdPdf pdf("pdf","pdf",RooArgList(pdf_de,pdf_mbc));

  RooAddPdf pdf_draw("pdf_draw","pdf_draw",RooArgList(pdf_sig,pdf_comb,pdf_bb_comb),RooArgList(Nsig,Ncmb,NBBcmb));

  RooArgSet* params = pdf.getParameters(RooArgSet(de,mbc));
//  RooArgset* initParams = (RooArgSet*) params->snapshot();

  pdf_mbc.fitTo(ds,Verbose(),Timer(true));
  ...
  params->printLatex(OutputFile("PurityEtaGGFit.tex"));
   ...
   RooAbsReal* intTot  =     pdf.createIntegral(RooArgSet(de,mbc),NormSet(RooArgSet(de,mbc)),Range("Signal"));
...
//   RooAbsReal* intCmb  = pdf_comb.createIntegral(RooArgSet(de,mbc),NormSet(RooArgSet(de,mbc)),Range("Signal"));
   ...
   const double ntot = intTot->getVal()*(Nsig.getVal()+Ncmb.getVal()+NBBcmb.getVal());
...
//   const double ncmb_err_total = TMath::Sqrt(ncmb_err*ncmb_err+ncmb_err_npq*ncmb_err_npq);
   ...
   const double purity_err = nsig_err_total/ntot;
   ...
   RooAbsReal* intTotEl =     pdf.createIntegral(RooArgSet(de,mbc),NormSet(RooArgSet(de,mbc)),Range("Ellips"));
...
//   RooAbsReal* intCmbEl = pdf_comb.createIntegral(RooArgSet(de,mbc),NormSet(RooArgSet(de,mbc)),Range("Ellips"));
   ...
   const double ntotEl = intTotEl->getVal()*(Nsig.getVal()+Ncmb.getVal()+NBBcmb.getVal());
...
//   const double ncmb_errEl_total = TMath::Sqrt(ncmb_errEl*ncmb_errEl+ncmb_errEl_npq*ncmb_errEl_npq);
   const double purityEl = nsigEl/ntotEl;
   const double purity_errEl = nsig_errEl_total/ntotEl;

   RooAbsReal* intSigEl1 = pdf_sig.createIntegral(RooArgSet(de,mbc),NormSet(RooArgSet(de,mbc)),Range("Elli"));
   RooAbsReal* intTotEl1 =     pdf.createIntegral(RooArgSet(de,mbc),NormSet(RooArgSet(de,mbc)),Range("Elli"));
...
//   RooAbsReal* intCmbEl1 = pdf_comb.createIntegral(RooArgSet(de,mbc),NormSet(RooArgSet(de,mbc)),Range("Elli"));
   ...
   const double ntotEl1 = intTotEl1->getVal()*(Nsig.getVal()+Ncmb.getVal()+NBBcmb.getVal());
...
//   const double ncmb_errEl1_total = TMath::Sqrt(ncmb_errEl1*ncmb_errEl1+ncmb_errEl1_npq*ncmb_errEl1_npq);
   const double purityEl1 = nsigEl1/ntotEl1;
   const double purity_errEl1 = nsig_errEl1_total/ntotEl1;
  ...
  pdf_draw.plotOn(deFrame,Components(pdf_sig),LineStyle(kDashed),ProjectionRange("mbcSignal"));
  pdf_draw.plotOn(deFrame,Components(RooArgSet(pdf_comb,pdf_bb_comb)),LineStyle(kDashed),ProjectionRange("mbcSignal"));
  pdf_draw.plotOn(deFrame,LineWidth(2),ProjectionRange("mbcSignal"));
  ...
  pdf_draw.plotOn(mbcFrame,Components(pdf_comb),LineStyle(kDashed),ProjectionRange("deSignal"));
  pdf_draw.plotOn(mbcFrame,Components(pdf_sig),LineStyle(kDashed),ProjectionRange("deSignal"));
  pdf_draw.plotOn(mbcFrame,Components(pdf_bb_comb),LineStyle(kDashed),ProjectionRange("deSignal"));
  pdf_draw.plotOn(mbcFrame,LineWidth(2),ProjectionRange("deSignal"));
