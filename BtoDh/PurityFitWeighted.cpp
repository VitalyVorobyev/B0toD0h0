...
void PurityFitWeighted(const int _mode){
...
//   mbc.setRange("Ellips",mbclo,mbchi);
  ...
  argset.add(b0f);
  RooDataSet dh("dh","dh",tree,argset,"mbc>0||mbc<=0");
...
//  RooDataSet ds_sig0("ds_sig0","ds_sig0",&ds_sig,RooArgSet(de,mbc));
//  RooDataSet ds_bkg0("ds_bkg0","ds_bkg0",&ds_bkg,RooArgSet(de,mbc));
//  RooDataHist* dh_sig = ds_sig0.binnedClone("dh_sig","dh_sig");
//  RooDataHist* dh_bkg = ds_bkg0.binnedClone("dh_bkg","dh_bkg");
//  RooDataHist dh;
//  dh.add(*dh_sig,"",1./0.563);
//  dh.add(*dh_bkg,"",1./0.949);
//  dh.add((RooAbsData)*dh_sig);
//  dh.add((RooAbsData)*dh_bkg);

  stringstream out;
  out.str("");
  out << "de<" << dE_max << " && de>" << dE_min;
  out << " && mbc>" << Mbc_min << " && mbc<" << Mbc_max;
  Roo1DTable* sigtable = dh.table(b0f,out.str().c_str());
  sigtable->Print();
  sigtable->Print("v");

  Roo1DTable* fulltable = dh.table(b0f);
  fulltable->Print();
  fulltable->Print("v");
...
//  RooArgset* initParams = (RooArgSet*) params->snapshot();
  ...
  dh.plotOn(deFrame,DataError(RooAbsData::SumW2),MarkerSize(1),CutRange("mbcSignal"));
  ...
  Roo1DTable* ellitable1 = dh.table(b0f,out.str().c_str());
  ...
  Roo1DTable* ellitable = dh.table(b0f,out.str().c_str());
...
//  out1 << "Purity: " << std::fixed << std::setprecision(2) << purity*100. << " #pm " << purity_err*100;
  ...
  dh.plotOn(mbcFrame,DataError(RooAbsData::SumW2),MarkerSize(1),CutRange("deSignal"));
...
//  elli->Draw(); elli1->Draw(); l1->Draw(); l2->Draw(); l3->Draw(); l4->Draw();
  ...
  out << "de<" << dE_max << " && de>" << dE_min;
  out << " && mbc>" << Mbc_min << " && mbc<" << Mbc_max;
  Roo1DTable* recttable = dh.table(b0f,out.str().c_str());
  recttable->Print();
  recttable->Print("v");
