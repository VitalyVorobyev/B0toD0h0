...
void ToyMCGenerator(const int nsamples = 1000){
...
//    RooGaussian backpdf("backpdf","backpdf",dt,mean,sigma);
...
//    RooBCPGenDecay pdf("pdf","pdf",dt,tag,tau,dm,avgMisgat,a,b,delMisgat,mu,rf,RooBCPGenDecay::DoubleSided);
    ...
    while(!ifile.eof()){
        getline(ifile,line);
        flag = sscanf(line.c_str(),"%lf %lf %lf %lf",&nsig,&pur,&signif,&cut);
        if(flag != 4) continue;
        cout << line << endl;
        int NSIG = (int)nsig;
//        vector<TTree*> treevec;
        ...
        for(int jj=0; jj<nsamples;jj++){
            ...
            out.str("");
            out << "ToyTree" << jj;
            TTree* tree = new TTree(out.str().c_str(),out.str().c_str());
...
//                a = (_K[i]-_Kb[i])/(_K[i]+_Kb[i]);
//                b = 2.*(_C[i]*_sin2beta+_S[i]*_cos2beta)*TMath::Sqrt(_K[i]*_Kb[i])/(_K[i]+_Kb[i]);
