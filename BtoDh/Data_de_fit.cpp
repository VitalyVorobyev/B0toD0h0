...
//if(flag41){ bdt_cut = 0.30;}

void Data_de_fit(void){
  TFile *ifile = TFile::Open("/home/vitaly/B0toDh0/TMVA/fil_b2dh_data.root");
...
//  RooFormulaVar _de("_de","_de","@0*@1",RooArgList(de,scale));
  ...
  RooRealVar c1("c1","c1",-4.27747e-01,-0.6,0.00001);
  RooRealVar c2("c2","c2",3.69914e-01,0.01,0.5);
  RooChebychev pdf_back("pdf_back","pdf_back",de,RooArgSet(c1,c2));

  RooRealVar c3("c3","c3",-3.69914e-01,-100,0.0);
//  RooExponential pdf_back("pdf_back","pdf_back",de,c3);
  ...
  TFile *ifile_sig = TFile::Open("/home/vitaly/B0toDh0/TMVA/fil_b2dh_sig.root");
  TTree *tree_sig = (TTree*)ifile_sig->Get("TEvent");
  RooArgSet argset_sig = argset;
//  RooCategory mcflag("mcflag","mcflag");
//  mcflag.defineType("signal",1);
//  mcflag.defineType("fsr",10);
//  mcflag.defineType("bad_pi0",5);
////  argset_sig.add(argset);
//  argset_sig.add(mcflag);
  ...
  TFile *ifile_back = TFile::Open("/home/vitaly/B0toDh0/TMVA/fil_b2dh_gen.root");
  ...
  argset_back.add(b0f);
  RooDataSet ds_back("ds_back","ds_back",tree_back,argset_back);
  if(!const_back_flag){
    pdf_back.fitTo(ds_back,Range(de_fit_min,de_fit_max),Timer(true));
  }
  out.str("");
  out << "de<" << de_max << " && de>" << de_min;
  const int gen_back = ds_back.sumEntries(out.str().c_str())/6.;
  ...
  if(const_back_flag){
    ...
    c2.setConstant(kTRUE);
//    pdf.fitTo(ds,Verbose(),Range(de_fit_min,de_fit_max),Timer(true));
//    pdf.fitTo(ds,Range(de_fit_min,de_fit_max),Timer(true));
    pdf.fitTo(ds,Range(-0.08,0.3),Timer(true));
  }
...
//  pdf.plotOn(deFrame,Components(g1b),LineStyle(kDashed));
...
//  de_lineDOWN->SetLineStyle(2);
  ...
  //  Gen MC  //
  //////////////
  RooPlot* deFrame_back = de.frame();
  ds_back.plotOn(deFrame_back,DataError(RooAbsData::SumW2),MarkerSize(1));
  pdf_back.plotOn(deFrame_back,LineWidth(2));
