...
//  RooProdPdf pdf_comb("pdf_comb","pdf_comb",RooArgList(pdf_de_comb,pdf_mbc_comb));
  ...
  RooRealVar pow2("pow2","pow2",m_pow2_bbcomb_eta10,0.1,10);                pow1.setConstant(kTRUE);
...
//  RooProdPdf pdf_bb_comb("pdf_bb_comb","pdf_bb_comb",RooArgList(pdf_mbc_bb_comb,pdf_de_bb_comb));
...
//  RooArgset* initParams = (RooArgSet*) params->snapshot();
  ...
  pdf.plotOn(deFrame,Components(pdf_de_sig),LineStyle(kDashed),ProjectionRange("mbcSignal"));
//  pdf.plotOn(deFrame,Components(pdf_bb_comb),LineStyle(kDashed),ProjectionRange("mbcSignal"));
  pdf.plotOn(deFrame,Components(pdf_de_comb),LineStyle(kDashed),ProjectionRange("mbcSignal"));
  ...
  pdf.plotOn(mbcFrame,Components(pdf_mbc_comb),LineStyle(kDashed),ProjectionRange("deSignal"));
  pdf.plotOn(mbcFrame,Components(pdf_mbc_sig),LineStyle(kDashed),ProjectionRange("deSignal"));
  pdf.plotOn(mbcFrame,Components(pdf_mbc_bb_comb),LineStyle(kDashed),ProjectionRange("deSignal"));
