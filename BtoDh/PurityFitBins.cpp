...
//  RooRealVar atckpi_max("atckpi_max","atckpi_max",0.,atckpi_cut); argset.add(atckpi_max);
  ...
  RooRealVar f_g("f_g","f_g",get_f_g_cmb_bb(_mode),0.,1.); if(!gg_flag) f_g.setConstant(kTRUE);
...
//    l_flv *= -1;
    ...
    NGoodEvents[flv_bin_tagv][bin_bin]++;
...
//    cout << bin_bin << " " << flv_bin << " " << flv_tagv << " " << flv_bin_tagv << endl;
  ...
  mg_sig->Draw("ap");
  pt1 = new TPaveText(0.65,0.80,0.98,0.98,"brNDC");
  pt1->SetFillColor(0);
  pt1->SetTextAlign(22);
  out.str("");
  out << label;
  ...
  pt1->Draw();
//  mg_sig->GetXaxis()->GetTitle("Dalitz & Flavor bin");
  ...
  c_sig->Update();
  out.str("");
  out << "pics/nsig_predicted_m" << _mode << ".eps";
  c_sig->Print(out.str().c_str());
  out.str("");
  out << "pics/nsig_predicted_m" << _mode << ".root";
  c_sig->Print(out.str().c_str());
...
//  RooDecay bkg_dt_pdf("bkg_dt_pdf","bkg_dt_pdf",dt,tau_bkg,delta_model,RooDecay::DoubleSided);
  RooAddPdf bkg_dt_peak("bkg_dt_peak","bkg_dt_peak",RooArgSet(dec_delta_pdf,dec_gauss_pdf),RooArgList(f_bkg_delta));

  RooRealVar s_otlr("s_otlr","s_otlr",get_s_otlr_bkg(_mode,55));
  RooRealVar f_otlr("f_otlr","f_otlr",get_f_otlr_bkg(_mode,55));
  RooGaussian bkg_dt_otlr("bkg_dt_otlr","bkg_dt_otlr",dt,RooConstVar("m_otlr","m_otlr",0),s_otlr);

  RooAddPdf bkg_dt_pdf("bkg_dt_pdf","bkg_dt_pdf",RooArgSet(bkg_dt_otlr,bkg_dt_peak),RooArgList(f_otlr));
  ...
  Double_t m_f_bkg;
  Int_t m_elli;
  Int_t k_mode, k_h0mode;

  tree_sig->SetBranchAddress("exp",&m_exp);
  tree_sig->SetBranchAddress("mode",&k_mode);
  tree_sig->SetBranchAddress("h0mode",&k_h0mode);
  ...
  toy_tree->Branch("mode",&k_mode,"mode/I");
  toy_tree->Branch("h0mode",&k_h0mode,"h0mode/I");
  ...
  for(int i=0; i<NSigTot; i++){
    ...
    tree_sig->GetEvent(i);
    if(k_mode != m_mode || k_h0mode != m_h0mode) continue;
    ...
    const int TagVFlv = m_tag > 0 ? 1 : 0;
    const int Flv_ind = flv_ind(TagVFlv);
    ...
    if(!sig_event){
      ...
      f_otlr.setVal(get_f_otlr_bkg(_mode,m_exp));
      s_otlr.setVal(get_s_otlr_bkg(_mode,m_exp)*10/cm2ps);
